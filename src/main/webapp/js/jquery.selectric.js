      ...
      Selectric = function(element, opts) {
        ...
        function _populate() {
          ...
          if ( optionsLength = $options.length ) {
			...
			//제품 셀렉트박스 숫자 뽑기 초기 세팅 2016-11-24
			var liDataSet = $itemsScroll.find("ul li");
			liDataSet.each(function(){
				var datan = $(this).text();
				var re_datan = datan.replace(/[^0-9,]/g,'');
				$(this).html(datan.replace(re_datan,'<em>'+re_datan+'</em>'));
			});

            $label.html(_this.items[currValue].text);
			var labelTextData = _this.items[currValue].text;
			var re_datan3 = labelTextData.replace(/[^0-9,]/g,'');
			$label.html(labelTextData.replace(re_datan3,'<em>'+re_datan3+'</em>'));
          }
        ...
        function _close(e) {
          ...
          if ( !e && currValue != selected ){
			...
			//제품 셀렉트박스 숫자 뽑기 선택시 세팅 2016-11-18
			var labelTextData = _this.items[currValue].text;
			var re_datan3 = labelTextData.replace(/[^0-9,]/g,'');
			$label.html(labelTextData.replace(re_datan3,'<em>'+re_datan3+'</em>'));
          }
